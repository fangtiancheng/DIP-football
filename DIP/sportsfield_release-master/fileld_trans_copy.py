...
opt.goal_image_path = '../trans_out/test_frame.jpg'
...
np_img = np.uint8(goal_image)
x_axis, y_axis = len(np_img[0]), len(np_img)
...
if opt.coord_conv_template:
    template_image = image_utils.rgb_template_to_coord_conv_template(
        template_image)
...
template_image_draw = image_utils.rgb_template_to_coord_conv_template(
    template_image_draw)
template_image_draw = utils.np_img_to_torch_img(template_image_draw)


# warp template image with optimized guess
warped_tmp_optim = warp.warp_image(
    template_image_draw, optim_homography, out_shape=outshape)[0]
...
overlay = (goal_image_draw[valid_index].astype(
    'float32') + warped_tmp_optim[valid_index].astype('float32'))/2
...
warped_frm = warp.warp_image(utils.np_img_to_torch_img(
    goal_image_draw)[None], H_inv, out_shape=outshape)[0]
plt.imshow(utils.torch_img_to_np_img(warped_frm)*0.5 +
           utils.torch_img_to_np_img(template_image_draw)*0.5)
plt.savefig("./after_trans.jpg")
plt.close('all')

# TODO:  this is needed to add into the new demo

# warp a point from frame to template
frame_point = np.array([954, 613])
plt.imshow(imageio.imread(opt.goal_image_path, pilmode='RGB'))
plt.scatter(frame_point[0], frame_point[1])
plt.savefig("./out_pre.jpg")
plt.close('all')

x = torch.tensor(frame_point[0] / x_axis - 0.5).float()
y = torch.tensor(frame_point[1] / y_axis - 0.5).float()
...
plt.savefig("../trans_out/demo_field_net.jpg")
plt.close('all')

pickle_data = [warped_frm, optim_homography]

with open("../trans_out/demo_field_out.pkl", "wb") as file:
