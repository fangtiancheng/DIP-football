...
while success and count < total_frame:
    ...
    for box in outeample_numpy:
        x_min, y_min, x_max, y_max = box
        x_cmp.append(x_min)
    x_cmp = np.array(x_cmp)
    seq = np.argsort(x_cmp)

    outeample_numpy = outeample_numpy[seq]
    human_score = human_score[seq]
    ...
    center_slide = []
    color_pair = []
    for idx in range(len(outeample_numpy)):
        x_min, y_min, x_max, y_max = outeample_numpy[idx].astype(int)
        center_x = (int)(x_min + x_max) // 2
        center_y = (int)(y_min + y_max) // 2
        mid_point = [center_y, center_x]
        center_slide.append(mid_point)
        color_clip = frame[y_min: y_max, x_min: x_max]
        color_list = np.average(color_clip, axis=1)
        color_list = np.average(color_list, axis=0).tolist()
    ...
    for idx in cluster[color_mode]:
        if mask[idx] == 0:
            continue
        fontsize = 2
        x_space = center_slide[idx][1]-19
        y_space = center_slide[idx][0]+19
        if idx >= 10:
            fontsize = 1
            y_space = y_space - 3
        cv2.circle(
            img, (center_slide[idx][1], center_slide[idx][0]), 30, (255, 255, 255), 2)
